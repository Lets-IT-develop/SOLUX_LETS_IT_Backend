...
public interface ProjectApi {
    ...
    Response<List<ProjectDto>> getOrganizingList(@Parameter(hidden = true) @CurrentUser Member member);

    @Operation(
            summary = "지원한 프로젝트 목록 조회",
            description = "현재 로그인한 사용자가 지원한 프로젝트 목록을 조회합니다."
    )
    Response<List<ProjectDto>> getAppliedList(@Parameter(hidden = true) @CurrentUser Member member);

    @Operation(
            summary = "진행 중인 프로젝트 목록 조회",
            description = "현재 로그인한 사용자가 참여 중인 프로젝트 목록을 조회합니다."
    )
    Response<List<OngoingProjectDto>> getOngoingList(@Parameter(hidden = true) @CurrentUser Member member);

    @Operation(
            summary = "완료된 프로젝트 목록 조회",
            description = "현재 로그인한 사용자가 완료한 프로젝트 목록을 조회합니다."
    )
    Response<List<OngoingProjectDto>> getCompletedList(@Parameter(hidden = true) @CurrentUser Member member);
}
